...
class SVMKernelClassifier:
    ...
    def train(self, DTR, LTR):
        ...
        (alfa, f, d) = scipy.optimize.fmin_l_bfgs_b(compute_lag, x0=numpy.zeros(LTR.size), approx_grad=False, factr=1.0,
                                                    bounds=bound_list)
        self.alfa = alfa
    ...
    def compute_scores(self, DTE):
        ...
        score = numpy.array(
            [self.compute_kernel_score(self.alfa, self.DTR, self.LTR, self.kernel_func, x, self.K * self.K, self.ci) for
             x in DTE.T])
        return score
