...
def ROC(thresholds, post_prob, LTE):
    ...
    FPR = []
    for t in thresholds:
        # Utilizzo t (e non prior) come valore di soglia per suddividere il dataset nei due set di classi
        pred = [1 if x >= t else 0 for x in post_prob]
        # Calcolo la matrice di confusione a partire da questi nuovi risultati
    ...
    plt.savefig(save_path + "ROCPlot.png")
    plt.close()
...
def Bayes_plot(llr, LTE):
    ...
    plt.legend()
    # plt.show()
    path = f'Images/Evaluation/Bayes/'
    os.makedirs(path, exist_ok=True)
    plt.title(f'Bayes')
    plt.savefig(path + "BayesPlot.png")
    plt.close()
